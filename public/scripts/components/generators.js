...
export function generatePdf() {
  ...
  const printArea = document.getElementById('print-template-container').innerHTML;
  const commision = document.getElementById('commission-name')
  const comment = document.getElementById('comment')
  // Otwórz nowe okno
  const doc = new jsPDF({
  ...
  const content = `
    <html>
      <head>
        <title>Drukuj zamówienie</title>
        <link rel="stylesheet" href="http://192.168.0.8/styles/order_details.css">
        <link rel="stylesheet" href="/styles/dialog.css">
        <link rel="stylesheet" href="/styles/main.css">
        <link rel="stylesheet" href="/scripts/components/toast.css">
  ...
  doc.html(content, {
    callback: function (doc) {
      doc.save(`${comment.dataset.id}"${commision.dataset.name}".pdf`);
    },
    x: 3,
    y: 0,
    width: 400,
