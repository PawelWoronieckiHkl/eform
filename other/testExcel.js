...
export class DialogManager {
    ...
    async initialize(param, options, groupNumber, filters) {
        ...
        this.isMultiChoice = param.MULTICHOICE || false;
        this.selectedValues = [];
    ...
    createSortingControls(onSortChange, currentSort = 'favorites') {
        const options = [
            { value: 'favorites', label: 'Ulubione na początku' },
            { value: 'az', label: 'A-Z' },
            { value: 'za', label: 'Z-A' }
        ];
    ...
    createImageWrapper(option, filename) {
        ...
        const imageSrc = `/data/${this.groupNumber}/${this.param.NAME}/${filename}`;
    ...
    handleOptionClick(clickedElement) {
        if (this.isMultiChoice) {
            ...
            // 4. NIE zamykamy okna - użytkownik może kontynuować wybór
        } else {
            // TRYB POJEDYNCZEGO WYBORU (ORYGINALNE ZACHOWANIE)
            // 1. Usuń zaznaczenie ze wszystkich elementów
            document.querySelectorAll('.image-box').forEach(e => e.classList.remove('active'));

            // 2. Zaznacz aktualny element
            clickedElement.classList.add('active');

            // 3. Natychmiastowe potwierdzenie i zamknięcie
            this.handleConfirm();
        }
    }
    ...
    handleConfirm() {
        // 1. Obsługa trybu MULTICHOICE
        if (this.isMultiChoice) {
            // Walidacja: czy wybrano co najmniej jedną opcję
            if (this.selectedValues.length === 0) {
                showToastInContainer(this.dialog, 'warning', t('form.empty_select_warning'));
                return;
            }

            // Łączenie wartości separatorem "|"
            const selectedValue = this.selectedValues.join('|');

            // Przekazanie danych
            const selectedData = {
                value: selectedValue,
                paramName: this.param.NAME,
                paramDescription: this.param.DESCRIPTION
            };

            // Wywołanie callbacku i zamknięcie dialogu
            if (this.callback) {
                this.callback(selectedData);
            }
            this.dialog.close();
        }
...
export function getInfoFromDialog(values, inputs, options, selectedData = null) {
    ...
    const result = updateFormWithSelectedValue(values, inputs, selectedData, options, 'siema');
